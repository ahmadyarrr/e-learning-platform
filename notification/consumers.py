...
class NotificationConsumer(AsyncWebsocketConsumer):
    ...
    async def connect(self):
        ...
        print("cnnneccced",self.group_name)
    ...
    async def receive(self, text_data=None, bytes_data=None):
        ...
        print('some data came to consumer')
        data = json.loads(text_data)
        if data["type"] == "share_notf":
            notf_id = int(data["notf_id"])
            notification = await Notification.objects.aget(id=notf_id)
            template = Template(
                '<li><a href="{{notification.related_object.get_absolute_url}}">{{notification.text}}</a></li>'
            )
